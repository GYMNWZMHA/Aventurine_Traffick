...
def generate_default_config():
    ...
    if screen_w == 1920 and screen_h == 1080:
        default_config = {
            ...
            "click_set_A": [
                {"name": "0", "capture_region": {"x": 1600, "y": 470, "w": 80, "h": 100}, "click": {"x": 1620, "y": 480, "delay": 0.5}},
                {"name": "1", "capture_region": {"x": 400, "y": 200, "w": 230, "h": 130}, "click": {"x": 600, "y": 120, "delay": 0.15}},
                {"name": "2", "capture_region": {"x": 650, "y": 200, "w": 230, "h": 130}, "click": {"x": 800, "y": 120, "delay": 0.15}},
                {"name": "3", "capture_region": {"x": 900, "y": 200, "w": 230, "h": 130}, "click": {"x": 950, "y": 120, "delay": 0.15}},
                {"name": "4", "capture_region": {"x": 1150, "y": 200, "w": 230, "h": 130}, "click": {"x": 1350, "y": 120, "delay": 0.15}},
                {"name": "5", "capture_region": {"x": 1400, "y": 200, "w": 230, "h": 130}, "click": {"x": 1550, "y": 120, "delay": 0.15}}
            ],
            "template_image_M": "target_M_1080p.png",
            "template_image_L1": "target_L1_1080p.png",
            "template_image_L2": "target_L2_1080p.png",
            "template_image_L3": "target_L3_1080p.png",
            "template_image_S1": "target_S1_1080p.png",
            "template_image_Sx": "target_Sx_1080p.png",
            "count_regions": [
                {"name": "DL", "x": 1300, "y": 950, "w": 50, "h": 18},
                {"name": "DR", "x": 1420, "y": 950, "w": 50, "h": 18},
                {"name": "U",  "x": 1360, "y": 750, "w": 80, "h": 90}
            ],
            "similarity_threshold": 0.8,
            "drag_set_B": {"from": {"x": 1300, "y": 900}, "to": {"x": 1800, "y": 900}, "duration": 0.15},
    ...
    print(Back.GREEN + f"已生成默认配置文件：{config_file} (分辨率 {screen_w}x{screen_h})")
    return True
...
def init_config_and_templates():
    ...
    def load_template(filename, name):
        # 构建完整路径
        full_path = os.path.join(script_dir, img_dir, filename)
        if os.path.exists(full_path):
            img = cv2.imread(full_path)
            if img is None:
                print(Back.RED + f"警告：无法读取 {full_path}，{name} 将失效")
                return None
            return img
        else:
            print(Back.RED + f"警告：图片文件不存在 {full_path}，{name} 将失效")
            return None
